...
target_lat6 = 37.525275 #6_dev1.rim 7_dev1.rim
target_lon6 = 139.937851
target_lat7 = 37.525355 #6_dev1.rim 7_dev1.rim
target_lon7 = 139.938932
target_lat8 = 37.525304 #6_dev1.rim 7_dev1.rim
target_lon8 = 139.939094
target_lat9 = 37.525310 #6_dev1.rim 7_dev1.rim
target_lon9 = 139.939045
target_lat10 = 37.525522 #8_dev2.rim
target_lon10 = 139.939111
target_lat11 = 37.525471 #8_dev1.rim
target_lon11 = 139.939047
...
def plot_rssi():
    ...
    plt.plot(distance,y,label='y=Alpha-10*Beta*np.log10(d)')
    plt.grid(True)
...
def ave():
    global rim_lon_x,rim_lat_y,rssi,ave_rssi,dic
    temp_r = np.array([])
    for i in range(len(rssi)):
        rsum = 0
        num = 0
        ...
        dic.append((distance[i],rsum/num,num))
    ave_rssi = temp_r
...
if __name__ == "__main__":
    ...
    #target_to_xy(target_lon2,target_lat2)
    #devtext_to_list('device/1_dev34_2.rim')
    ...
    #target_to_xy(target_lon6,target_lat6)
    #devtext_to_list('device/1_dev33.rim')
    target_to_xy(target_lon7,target_lat7)
    devtext_to_list('device/45_dev916.rim')
    #target_to_xy(target_lon8,target_lat8)
    #devtext_to_list('device/67_dev2.rim')
    #target_to_xy(target_lon9,target_lat9)
    #devtext_to_list('device/67_dev1.rim')
    #target_to_xy(target_lon10,target_lat10)
    #devtext_to_list('device/8_dev2.rim')
    #target_to_xy(target_lon11,target_lat11)
    #devtext_to_list('device/8_dev1.rim')
    ...
    for i in range(len(dic)):
        print "{0} {1}".format(dic[i][0],dic[i][1])
        #print "{0} {1} {2}".format(dic[i][0],dic[i][1],dic[i][2])
    #plot_rssi()
